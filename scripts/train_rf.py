...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / 'data' / 'output' / 'rf'
...
def parse_args():
    parser = argparse.ArgumentParser(description='Train RF model and invert chlorophyll-a from remote sensing imagery.')
...
def main():
    ...
    output_tif_path = output_dir / 'rf_result.tif'
    scatter_plot_path = output_dir / 'rf_scatter.png'
    result_map_path = output_dir / 'rf_result_map.png'
    ...
    if target_col not in df.columns:
        ...
        if possible_cols:
            ...
            print(f"提示: 未找到列名 'Chla'，自动匹配到目标列: {target_col}")
        else:
            raise ValueError(f'CSV中未找到叶绿素浓度列，请检查列名。现有列: {df.columns.tolist()}')
    ...
    print('    正在进行超参数网格搜索(GridSearch)，请稍候...')
    ...
    plt.scatter(y_test, y_pred, color='blue', alpha=0.6, label='Samples')
    ...
    with rasterio.open(img_path) as src:
        ...
        print(f'    原影像波段数: {src.count}')
        print('    正在读取影像数据 (仅提取前7个波段)...')
